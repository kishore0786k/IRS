...
- The proposed secrecy curve changes from 12.561 to 10.164 bps/Hz across the tested CSI-error range.
- The literature-inspired AO curve is included so the robustness claim is not limited to a greedy-only comparison; at the default point the proposed solver remains 13.26% stronger than AO in secrecy.

## figure_09_gain_vs_greedy
- The gain-over-greedy curve stays centered around an average of 15.94% over the tested CSI-error range.
...
- The proposed solver outperforms the legitimate-only AO baseline by -1.19% in sum-rate and 13.26% in secrecy at the default setting.
- Relative to greedy alignment, the proposed method shows -2.35% rate gain and 13.86% secrecy gain; discuss this pair together instead of relying on only one scalar metric.
