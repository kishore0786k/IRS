...
- The proposed secrecy curve changes from 11.659 to 9.092 bps/Hz across the tested CSI-error range.
- The literature-inspired AO curve is included so the robustness claim is not limited to a greedy-only comparison; at the default point the proposed solver remains 13.95% stronger than AO in secrecy.

## figure_09_gain_vs_greedy
- The gain-over-greedy curve stays centered around an average of 16.09% over the tested CSI-error range.
...
- The proposed solver outperforms the legitimate-only AO baseline by -0.99% in sum-rate and 13.95% in secrecy at the default setting.
- Relative to greedy alignment, the proposed method shows -2.45% rate gain and 13.76% secrecy gain; discuss this pair together instead of relying on only one scalar metric.
